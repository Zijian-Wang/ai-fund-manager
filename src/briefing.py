...
- 考虑T+1交易规则。
- 交易数量为100股的整数倍。
- **单只标的最大持仓 ≤ 50% of NAV**（初始 ¥100,000，即任一标的买入后总市值不得超过 ¥50,000）。
- **你不知道当前实时成交价**。如果你要 BUY 具体个股/ETF，必须：
  (a) 用你能用的工具（web search 等）先查收盘价，或
  (b) 在 `reason.thesis` 里显式写出"基于收盘价约 ¥X"的假设，并据此计算数量。
  不要猜价格后再反推数量——真实价格 × 你的数量如果超过 50% NAV，决策会被 guardrail 拒绝。
  典型价格参考：600/601 开头大盘股常见 ¥10-¥2000 范围；300308 这类光模块龙头 ¥500-¥1000；
  510/512/588 开头的 ETF 通常 ¥1-¥3；159 开头的 ETF 通常 ¥1-¥5。

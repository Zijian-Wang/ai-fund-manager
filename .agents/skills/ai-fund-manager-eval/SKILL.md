...
- `.env` has `TUSHARE_TOKEN`. GEMINI_API_KEY and OLLAMA_CLOUD_API_KEY are used for the API agents in this book.

## Supported agents (manual mode)

Default agents (fresh 600k book, focus on open models via Ollama Cloud + Gemini API):
- **qwen** (Ollama Cloud — Qwen 3.5)
- **deepseek** (Ollama Cloud — DeepSeek R1)
- **glm** (Ollama Cloud — GLM-5.2 latest)
- **gemini** (Google Gemini API — Gemini 3.1 Pro Thinking / latest)

These are now powered by the latest models found (Ollama for qwen/glm; Gemini key for gemini).
The user can skip any per run.
...
> Which agents today? (default: qwen, deepseek, glm, gemini)

Wait for the answer. Store the list as `AGENTS` for the rest of the run.
If the user says "all" or just presses enter, use ["qwen", "deepseek", "glm", "gemini"].
...
AGENTS = ["qwen", "deepseek", "glm", "gemini"]  # new book focus: Ollama open models + Gemini API
...
AGENTS = ["qwen", "deepseek", "glm", "gemini"]  # per user choice
...
AGENTS = ["qwen", "deepseek", "glm", "gemini"]
eval_date = "<EVAL_DATE>"
cache_root = Path("data_cache"); agents_root = Path("agents")
...
DISPLAY_NAMES = {
    "qwen": "Qwen 3.5 (Ollama Cloud)",
    "deepseek": "DeepSeek R1 (Ollama Cloud)",
    "glm": "GLM-5.2 (Ollama Cloud)",
